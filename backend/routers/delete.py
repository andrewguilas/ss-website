...
def get_first_route_with_orders(db, truck_id):
    ...
    for route in routes:
        has_orders = db.query(Order).filter(Order.route_id == route.id).first()
        if has_orders:
            return route.id
...
def delete_truck(truck_id: int, db: Session = Depends(get_db)):
    ...
    if first_route_id_with_orders:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot delete truck {truck_id}: Route {first_route_id_with_orders} has assigned orders."
        )
